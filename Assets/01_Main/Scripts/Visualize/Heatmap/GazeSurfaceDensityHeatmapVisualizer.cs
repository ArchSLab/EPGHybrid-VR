...
public class GazeSurfaceDensityHeatmapVisualizer : MonoBehaviour
{
    ...
    public Rect panelRect = new Rect(20, 20, 360, 620);
    public float nodeListHeight = 260f;

    [Header("Record Filtering")]
    public int sampleStep = 1;
    public int maxRecords = 20000;
    public string hitObjectTagFilter = "";       // empty or All = no filter, e.g. Signage, Wall, Floor
    public string hitObjectNameContains = "";    // empty = no filter
    public int minRecordsPerSurface = 3;
    public int maxSurfaces = 200;
    ...
    private void OnGUI()
    {
        ...
        panelRect = GUI.Window(2026070801, panelRect, DrawPanel, "");
    }

    private void DrawPanel(int windowID)
    {
        GUILayout.BeginHorizontal(GUILayout.Height(28));
        GUILayout.Label("Surface Density Heatmap", GUILayout.Width(220));
        ...
        taskOrder = IntTextField(taskOrder, 1, 99999, GUILayout.Width(80));
        GUILayout.Label("CSV Index:", GUILayout.Width(90));
        csvFileIndex = IntTextField(csvFileIndex, 1, 99999, GUILayout.Width(80));
        GUILayout.EndHorizontal();
        ...
        else if (!normalizePerSurface)
        {
            manualMaxDensity = FloatFieldWithSlider("Manual Max Density", manualMaxDensity, 0.1f, 500f);
        }
        else
        {
            manualMaxDensity = FloatFieldWithSlider("Manual Max Density", manualMaxDensity, 0.1f, 500f);
        ...
        nodeListHeight = GUILayout.HorizontalSlider(nodeListHeight, 120f, 650f, GUILayout.Width(180));
    ...
    private int IntTextField(int value, int min, int max, params GUILayoutOption[] options)
    {
    ...
    private int IntField(string label, int value, int min, int max)
    {
        ...
        value = IntTextField(value, min, max, GUILayout.Width(85));
        GUILayout.EndHorizontal();
        return value;
    }
    ...
    private float ColorSlider(string label, float value)
    {
        ...
        value = GUILayout.HorizontalSlider(value, 0f, 1f, GUILayout.Width(220));
    ...
    private void LoadCsvAndBuildNodeStays()
    {
        ...
        if (idxNodeID < 0 || idxHitX < 0 || idxHitY < 0 || idxHitZ < 0)
        {
            statusMessage =
                ...
                "Recommended: HitNormalX/Y/Z, HitObjectName, HitObjectTag, HitObjectVariant, HitColliderPath.\n" +
                csvPath;
            Debug.LogError(statusMessage);
            return;
        }
        ...
        for (int i = 1; i < table.Count; i++)
        {
            ...
            rec.hitObjectVariant = idxHitObjectVariant >= 0 ? SafeGet(row, idxHitObjectVariant) : "";
            rec.hitColliderPath = idxHitColliderPath >= 0 ? SafeGet(row, idxHitColliderPath) : "";
    ...
    private string GetTargetCsvPath()
    {
        ...
        string eyeFolder = Path.Combine(Path.Combine(processedRoot, "Task" + taskOrder), "Eye");
    ...
    private void RefreshHeatmap()
    {
        ...
        if (groups.Count == 0)
        {
            statusMessage = "No surface groups were created. Check filters and CSV fields.";
            Debug.LogWarning(statusMessage);
            return;
        }
        ...
        if (renderDataList.Count == 0)
        {
            statusMessage = "No valid surfaces after grouping.\n" +
                            "Groups: " + groups.Count + " | Skipped small groups: " + skippedSmallGroups;
            Debug.LogWarning(statusMessage);
            return;
        }
        ...
        heatmapRoot = new GameObject("SurfaceDensityHeatmap_Task" + taskOrder);
        ...
        statusMessage =
            ...
            "Points used in created surfaces: " + totalPoints + "\n" +
            "Skipped small groups: " + skippedSmallGroups + "\n" +
            "Normalize per surface: " + normalizePerSurface + "\n" +
            "Global clamp: " + globalClamp.ToString("F3", CultureInfo.InvariantCulture);
    ...
    private List<GazeRecord> CollectSelectedHitRecords()
    {
        ...
        int limit = maxRecords <= 0 ? int.MaxValue : maxRecords;
        ...
        for (int s = 0; s < nodeStays.Count; s++)
        {
            ...
            for (int i = seg.startRecordIndex; i <= seg.endRecordIndex; i += step)
            {
                ...
                if (useTagFilter && !string.Equals(rec.hitObjectTag, tagFilter, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
                ...
                if (useNameFilter)
                {
                    string objectName = rec.hitObjectName == null ? "" : rec.hitObjectName;
    ...
    private Dictionary<string, SurfaceGroup> BuildSurfaceGroups(List<GazeRecord> selectedHits)
    {
        ...
        for (int i = 0; i < selectedHits.Count; i++)
        {
            ...
            string key = BuildSurfaceKey(rec);
            ...
            if (!groups.TryGetValue(key, out group))
            {
                ...
                group.key = key;
                group.objectName = rec.hitObjectName;
                group.objectTag = rec.hitObjectTag;
                group.objectVariant = rec.hitObjectVariant;
                group.colliderPath = rec.hitColliderPath;
    ...
    private SurfaceRenderData BuildSurfaceRenderData(SurfaceGroup group)
    {
        ...
        {
            averageNormal += group.records[i].hitNormal;
            averagePoint += group.records[i].hitPoint;
        }

        averagePoint /= Mathf.Max(1, group.records.Count);
        ...
        for (int i = 0; i < group.records.Count; i++)
        {
            Vector3 delta = group.records[i].hitPoint - averagePoint;
            float u = Vector3.Dot(delta, uAxis);
            float v = Vector3.Dot(delta, vAxis);
            localPoints.Add(new Vector2(u, v));
        ...
        {
            float center = (minU + maxU) * 0.5f;
            minU = center - minSurfaceSize * 0.5f;
            maxU = center + minSurfaceSize * 0.5f;
        ...
        if (height <= minSurfaceSize)
        {
            ...
            minV = center - minSurfaceSize * 0.5f;
            maxV = center + minSurfaceSize * 0.5f;
        ...
        float localClamp = usePercentileClamp
            ? ComputePercentile(density, Mathf.Clamp01(clampPercentile))
            : Mathf.Max(0.0001f, manualMaxDensity);
    ...
    private float[] RasterizeDensity(
        ...
        for (int p = 0; p < localPoints.Count; p++)
        {
            ...
            for (int y = y0; y <= y1; y++)
            {
                ...
                for (int x = x0; x <= x1; x++)
                {
                    ...
                    if (d2 <= radius * radius)
                    {
                        density[y * resolution + x] += Mathf.Exp(-d2 / twoSigma2);
                    }
                }
            }
        }
    ...
    public void ConfigureFromReplay(string root, int task, int csvIndex, string overridePath)
    {
        processedRoot = root;
        taskOrder = task;
